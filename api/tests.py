...
class TestRunningGroupViewSet(APITestCase):
    ...
    def setUp(self):
        ...
        self.credentials1 = {
            'username': 'testuser',
            'password': 'testpassword'}
        MyUser.objects.create_user(**self.credentials1)

        self.credentials2 = {
            'username': 'testuser2',
            'password': 'testpassword'}
        MyUser.objects.create_user(**self.credentials2)
...
class TestPastTrainingViewSet(APITestCase):
    ...
    def setUp(self):
        ...
        PastTraining.objects.create(**self.credentials3)
    ...
    def test_delete_past_training(self):
        ...
        self.client.login(**self.credentials2)
        pasttrainings_id = PastTraining.objects.first().id
        response = self.client.delete(f"/api/v1/pasttrainings/{pasttrainings_id}/", content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
